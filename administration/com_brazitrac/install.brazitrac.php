...
//	$database->setQuery( "UPDATE #__menu SET admin_menu_img=\"".$icon."\" WHERE " . $database->nameQuote('title') . "=\"".$name."\" AND " . $database->nameQuote('option') . "=\"com_brazitrac\";");
//	$database->query();
//}
...
function com_install()
{
	...
	if ( $database->getErrorNum() == 0 )
	{
		...
		if ( count( $vars ) == 1 )
		{
			...
			{
				$upgrade = true;
				$database->setQuery( "SELECT value FROM #__brazitrac_settings WHERE name=\"versionmajor\"" );
				$vars = $database->loadObjectList();
				$version = $vars[0]->value;
				$database->setQuery( "SELECT value FROM #__brazitrac_settings WHERE name=\"versionminor\"" );
				$vars = $database->loadObjectList();
				$version .= '.'.$vars[0]->value;
				$database->setQuery( "SELECT value FROM #__brazitrac_settings WHERE name=\"versionpatch\"" );
				$vars = $database->loadObjectList();
				$version .= '.'.$vars[0]->value;
			}
		}
	...
	switch ( $version ) {
		 ...
		 */	
		case '0.0.0':
			$database->setQuery( "DROP TABLE IF EXISTS " . $database->nameQuote('#__brazitrac_category') . ";");
			$database->query();
			...
			$database->query();
			$database->setQuery( "DROP TABLE IF EXISTS " . $database->nameQuote('#__brazitrac_groups') . ";");
			$database->query();
			$database->setQuery( "CREATE TABLE  " . $database->nameQuote('#__brazitrac_groups') . " (
			...
			$database->query();
			$database->setQuery( "DROP TABLE IF EXISTS " . $database->nameQuote('#__brazitrac_permissions') . ";");
			$database->query();
			$database->setQuery( "CREATE TABLE  " . $database->nameQuote('#__brazitrac_permissions') . " (
			...
			$database->query();
			$database->setQuery( "DROP TABLE IF EXISTS " . $database->nameQuote('#__brazitrac_settings') . ";");
			$database->query();
			$database->setQuery( "CREATE TABLE  " . $database->nameQuote('#__brazitrac_settings') . " (
			...
			$database->query();			
			$database->setQuery( "INSERT INTO " . $database->nameQuote('#__brazitrac_settings') . " (" . $database->nameQuote('name') . "," . $database->nameQuote('value') . ") VALUES ('iconset','mdn_'),
                                                                                               ('highlight','!'),
                                                                                               ('enhighlight','1'),
                                                                                               ('ticketsfront','5'),
                                                                                               ('ticketssub','15'),
			...
			$database->query();
			$database->setQuery( "INSERT INTO " . $database->nameQuote('#__brazitrac_category') . " (" . $database->nameQuote('catid') . "," . $database->nameQuote('name') . "," . $database->nameQuote('description') . "," . $database->nameQuote('image') . ") VALUES (1,'Default Category','If there are no other suitable categories submit your tickets here ;)',NULL);");			
			$database->query();
			break;
		/**
		 ...
		 */	
		case '2.0.0':
			$database->setQuery( "INSERT INTO " . $database->nameQuote('#__brazitrac_settings') . " (" . $database->nameQuote('name') . "," . $database->nameQuote('value') . ") VALUES ('   ','0'),('debugmessage','Continue >>');");
			$database->query();
			break;			
		/**
		...
		case '3.0.3':
            ...
            $database->setQuery( "ALTER TABLE " . $database->nameQuote('#__brazitrac_ticket') . " MODIFY COLUMN " . $database->nameQuote('ticketname') . " VARCHAR(255) NOT NULL;" );
			$database->query();            
            // convert timestamp fields to datetime fields
            $database->setQuery('ALTER TABLE ' . $database->nameQuote('#__brazitrac_highlight') . ' MODIFY COLUMN ' . $database->nameQuote('datetime') . ' DATETIME NOT NULL;');
            $database->query();
            $database->setQuery('ALTER TABLE ' . $database->nameQuote('#__brazitrac_ticket') . ' MODIFY COLUMN ' . $database->nameQuote('datetime') . ' DATETIME NOT NULL;');
            $database->query();
            $database->setQuery('ALTER TABLE ' . $database->nameQuote('#__brazitrac_msg') . ' MODIFY COLUMN ' . $database->nameQuote('datetime') . ' DATETIME NOT NULL;');
            $database->query();
            $database->setQuery("UPDATE " . $database->nameQuote('#__brazitrac_settings') . " SET " . $database->nameQuote('value') . " = '%d-%m-%Y (%H:%M)' WHERE " . $database->nameQuote('name') . " = 'date'");
            $database->query();
            $database->setQuery("UPDATE " . $database->nameQuote('#__brazitrac_settings') . " SET " . $database->nameQuote('value') . " = '%d-%m-%Y' WHERE " . $database->nameQuote('name') . " = 'dateshort'");
            $database->query();
            break;
        /**
		...
		case '3.0.4':
            ...
            $database->setQuery( "ALTER TABLE " . $database->nameQuote('#__brazitrac_category') . " ADD COLUMN " . $database->nameQuote('emails') . " VARCHAR(255) NOT NULL;" );
			$database->query();            
	}
    
    // set the version
    $database->setQuery('UPDATE ' . $database->nameQuote('#__brazitrac_settings') . 
